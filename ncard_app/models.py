...
# ORCID ident. format based on https://support.orcid.org/hc/en-us/articles/360006897674-Structure-of-the-ORCID
# -Identifier
orcid_validator = RegexValidator(r'^$|^https://orcid\.org/\d{4}-\d{4}-\d{4}-\d{3}(\d|X)$',
                                 'ORCID identifier must be a full URL, in this format: '
                                 'https://orcid.org/XXXX-XXXX-XXXX-XXXX')
# https://help.twitter.com/en/managing-your-account/twitter-username-rules
twitter_validator = RegexValidator(r'^$|^@[A-Za-z0-9_]+$',
                                   'Twitter handle must begin with an @ and only contain letters, digits and '
                                   'underscores.')
...
class Organisation(models.Model):
    ...
    primary_contact = models.ForeignKey('ncard_app.Person', on_delete=models.RESTRICT, null=True, blank=True,
...
class Department(models.Model):
    name = models.CharField('name', max_length=255)
    org = models.ForeignKey(Organisation, verbose_name='Organisation', on_delete=models.RESTRICT,
                            related_name='org_depts')
...
class Person(models.Model):
    ...
    organisation_primary = models.ForeignKey(Organisation, on_delete=models.SET_NULL, null=True, blank=True,
                                             related_name='contacts_primary_org', verbose_name='Organisation (Primary)')
    department_primary = models.ForeignKey(Department, on_delete=models.SET_NULL, null=True, blank=True,
                                           related_name='contacts_primary_dept', verbose_name='Department (Primary)')
    organisation_other = models.ForeignKey(Organisation, on_delete=models.SET_NULL, null=True, blank=True,
                                           related_name='contacts_other_org', verbose_name='Organisation (Other)')
    department_other = models.ForeignKey(Department, on_delete=models.SET_NULL, null=True, blank=True,
                                         related_name='contacts_other_dept', verbose_name='Department (Other)')
    ...
    work_country = models.ForeignKey(Country, on_delete=models.RESTRICT, to_field='code', default='AU',
                                     related_name='+', verbose_name='Country')
    ...
    home_country = models.ForeignKey(Country, on_delete=models.RESTRICT, to_field='code', default='AU',
                                     related_name='+', verbose_name='Country')
...
class Award(models.Model):
    ...
    no_year = models.DecimalField(verbose_name="Concurrent Years", default=1.0, max_digits=10, decimal_places=1,
...
class Publication(models.Model):
    ...
    issue = models.PositiveSmallIntegerField('issue', blank=True, null=True)
...
class Grant(models.Model):
    ...
    total_request = models.DecimalField(verbose_name="Total Amount Requested", max_digits=15, decimal_places=2,
                                        null=True, blank=True)
    total_award = models.DecimalField(verbose_name="Total Amount Awarded", max_digits=15, decimal_places=2, null=True,
                                      blank=True)
    status = models.IntegerField('Status', null=True, blank=True, choices=GrantStatus.choices,
